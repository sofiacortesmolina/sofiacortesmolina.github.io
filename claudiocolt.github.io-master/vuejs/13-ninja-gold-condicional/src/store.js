export default {
  ...
  log(val, location){
    ...
    if(val > 0){
      ...
      this.data.activity.unshift({
        color: `blue`,
        texto: `- Earned <b>${val} gold</b> from the <b>${location}</b> 💰 (${fecha})`
      });
      ...
      this.data.activity.unshift({
        color: `red`,
        texto: `- Lost <b>${val} gold</b> from the <b>${location}</b> 💸 (${fecha})`
      });
      ...
      this.data.activity.unshift({
        color: `gray`,
        texto: `- Got <b>${val} gold</b> from the <b>${location}</b> 😞 (${fecha})`
      });
  ...
  resetGold(){
    ...
    if(confirm("Borrar y Comenzar de nuevo?")){
      this.data.gold  = 0;
      this.data.activity.length = 0;
    }
